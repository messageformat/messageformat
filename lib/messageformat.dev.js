...
(function ( root ) {
  ...
  MessageFormat.format = {
    number: function(currency) {
      return new Function("v,lc,p",
        "return Intl.NumberFormat(lc,\n" +
        "    p=='integer' ? {maximumFractionDigits:0}\n" +
        "  : p=='percent' ? {style:'percent'}\n" +
        "  : p=='currency' ? {style:'currency', currency:'" + currency + "', minimumFractionDigits:2, maximumFractionDigits:2}\n" +
        "  : {}).format(v)"
      );
    },
    ...
    function interpMFP ( ast, data ) {
      ...
      switch ( ast.type ) {
        ...
        case 'messageFormatElement':
          ...
          return '';
        case 'elementFormat':
          tmp = [ 'd[' + data.keys[data.pf_count] + ']' ];
          if ( ast.key === 'select' ) {
            tmp.push(interpMFP(ast.val, data));
            return '_s(' + tmp.join(',') + ')';
          }
          else if ( ast.key === 'selectordinal' ) {
            tmp = tmp.concat([ 0, 'pf[' + JSON.stringify(self.lc[0]) + ']', interpMFP(ast.val, data), 1 ]);
            return '_p(' + tmp.join(',') + ')';
          }
          else if ( ast.key === 'plural' ) {
            data.offset[data.pf_count || 0] = ast.val.offset || 0;
            tmp = tmp.concat([ data.offset[data.pf_count] || 0, 'pf[' + JSON.stringify(self.lc[0]) + ']', interpMFP(ast.val, data) ]);
            return '_p(' + tmp.join(',') + ')';
          }
          else {
            tmp.push(JSON.stringify(self.lc));
            if (ast.val && ast.val.length) tmp.push(ast.val.length == 1 ? ast.val[0] : ast.val);
            return 'fmt.' + ast.key + '(' + tmp.join(',') + ')';
          }
          return '';
        /* // Unreachable cases.
        case 'pluralStyle':
        ...
        */
        case 'string':
          ...
          if ( data.pf_count ) {
            ...
            tmp = tmp.replace(/(^|[^\\])#/g, '$1"+' + '_n(d[' + data.keys[data.pf_count-1] + ']' + (o ? ',' + o : '') + ')+"');
