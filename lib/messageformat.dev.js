...
(function ( root ) {
  ...
  MessageFormat.Utils = {
    numSub : function ( string, d, key, offset ) {
      ...
      var s = string.replace( /(^|[^\\])#/g, '$1"+_MF.n(' + d + ',' + key + (offset ? ',' + offset : '') + ')+"' );
    ...
    function interpMFP ( ast, data ) {
      ...
      switch ( ast.type ) {
        ...
        case 'messageFormatElement':
          ...
          if ( ast.output ) {
            return '_MF.v(d,"' + ast.argumentIndex + '")';
          }
          else {
        ...
        case 'elementFormat':
          if ( ast.key === 'select' ) {
            return '_MF.s(d,' + data.keys[data.pf_count] + ',' + interpMFP( ast.val, data ) + ')';
          }
          else if ( ast.key === 'plural' ) {
            data.offset[data.pf_count || 0] = ast.val.offset || 0;
            return '_MF.p(d,' + data.keys[data.pf_count] + ',' + (data.offset[data.pf_count] || 0)
        ...
        case 'pluralFormatPattern':
          ...
          for ( i = 0; i < ast.pluralForms.length; ++i ) {
            ...
            r.push('"' + ast.pluralForms[ i ].key + '":' + interpMFP( ast.pluralForms[ i ].val,
