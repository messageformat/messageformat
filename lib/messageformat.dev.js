...
(function ( root ) {
  ...
  MessageFormat.num = function(x, key){
      ...
      switch ( ast.type ) {
        ...
        case 'elementFormat':
          if ( ast.key === 'select' ) {
            ...
            s += 'r += (pf_' +
                 data.pf_count +
                 '[ d['+data.keys[data.pf_count]+'] ] || pf_'+data.pf_count+'[ "other" ])( d );\n';
          }
          else if ( ast.key === 'plural' ) {
            s += interpMFP( ast.val, data );
            s += 'r += MessageFormat.plural(pf_' + data.pf_count + ',' + data.keys[data.pf_count] + ',off_' + data.pf_count + ',d,"' + self.fallbackLocale + '");\n';
          }
          return s;
        /* // Unreachable cases.
        case 'pluralStyle':
        ...
        case 'string':
          ...
          if ( data.pf_count ) {
            tmp = MessageFormat.Utils.numSub( tmp,
              'd[' + data.keys[data.pf_count-1] + '] - off_' + ( data.pf_count - 1 ),
