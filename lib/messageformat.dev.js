    ...
    function interpMFP ( ast, data ) {
      ...
      switch ( ast.type ) {
        ...
        case 'messageFormatElement':
          ...
          if ( ast.output ) {
            s += 'r += _MF.val(d,"' + ast.argumentIndex + '");\n';
          }
          else {
        ...
        case 'elementFormat':
          if ( ast.key === 'select' ) {
            s += 'r += _MF.sel(d,' + data.keys[data.pf_count] + ',' + interpMFP( ast.val, data ) + ');\n';
          }
          else if ( ast.key === 'plural' ) {
            data.offset[data.pf_count || 0] = ast.val.offset || 0;
            s += 'r += _MF.plu(d,' + data.keys[data.pf_count] + ',' + (data.offset[data.pf_count] || 0)
