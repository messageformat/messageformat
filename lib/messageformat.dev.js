...
(function ( root ) {
  ...
  MessageFormat.num = function(d, k, o){
    if(isNaN(d[k])){throw new Error("MessageFormat: `"+k+"` isnt a number.")}
    return d[k] - (o || 0);
  };
  MessageFormat.val = function(d, k){
    MessageFormat.check_d(d);
    return d[k];
  },
  MessageFormat.plural = function(d, k, o, l, p){
    MessageFormat.check_d(d);
    ...
    function interpMFP ( ast, data ) {
      ...
      switch ( ast.type ) {
        ...
        case 'messageFormatElement':
          ...
          if ( ast.output ) {
            s += 'r += MessageFormat.val(d,"' + ast.argumentIndex + '");\n';
          }
          else {
        ...
        case 'elementFormat':
          if ( ast.key === 'select' ) {
            s += 'r += MessageFormat.select(d,' + data.keys[data.pf_count] + ',' + interpMFP( ast.val, data ) + ');\n';
          }
          else if ( ast.key === 'plural' ) {
            data.offset[data.pf_count || 0] = ast.val.offset || 0;
            s += 'r += MessageFormat.plural(d,' + data.keys[data.pf_count] + ',' + (data.offset[data.pf_count] || 0)
