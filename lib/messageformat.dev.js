  ...
  MessageFormat.Utils = {
    numSub : function ( string, d, key, offset ) {
      ...
      return string.replace( /^#|[^\\]#/g, function (m) {
        ...
        return prefix + '" + _MF.num(' + d + ',' + key + (offset ? ',' + offset : '') + ') + "';
      });
    },
    ...
    function interpMFP ( ast, data ) {
      ...
      switch ( ast.type ) {
        ...
        case 'messageFormatElement':
          ...
          if ( ast.output ) {
            return '_MF.val(d,"' + ast.argumentIndex + '")';
          }
          else {
        ...
        case 'elementFormat':
          if ( ast.key === 'select' ) {
            return '_MF.sel(d,' + data.keys[data.pf_count] + ',' + interpMFP( ast.val, data ) + ')';
          }
          else if ( ast.key === 'plural' ) {
            data.offset[data.pf_count || 0] = ast.val.offset || 0;
            return '_MF.plu(d,' + data.keys[data.pf_count] + ',' + (data.offset[data.pf_count] || 0)
