 ...
 *
 *  If `src` is an object with a key that is also present in `pluralCategories`,
 *  the * key in question will be used as the locale identifier for its value.
 *
 * @param {string|object} src - the source for which the JS code should be generated
 * @param {string} lc - the default locale
 * @param {object} pluralCategories - a map of pluralization keys for all available locales
 */
Compiler.prototype.compile = function(src, lc, pluralCategories) {
  if (typeof src != 'object') {
    this.lc = lc;
    var pc = pluralCategories[lc] || {};
    var r = parse(src, pc).map(function(token) { return this.token(token); }, this);
    return 'function(d) { return ' + (r.join(' + ') || '""') + '; }';
  } else {
    var result = {};
    for (var key in src) {
      var lcKey = pluralCategories.hasOwnProperty(key) ? key : lc;
      result[key] = this.compile(src[key], lcKey, pluralCategories);
    }
    return result;
  }
}
