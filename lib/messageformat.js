...
MessageFormat.prototype._compileCases = function(ast, lc, data) {
  var r = [];
  var needOther = true;
  for (var i = 0; i < ast.cases.length; ++i) {
    var key = ast.cases[i].key;
    if (key === 'other') needOther = false;
    var data_copy = JSON.parse(JSON.stringify(data));
    data_copy.pf_count++;
    var s = [];
    for (var j = 0; j < ast.cases[i].tokens.length; ++j) {
      s.push(this._compileToken(ast.cases[i].tokens[j], lc, data_copy));
    }
    r.push(propname(key) + ': function() { return ' + (s.join(' + ') || '""') + ';}');
  }
 ...
 * @private
 */
MessageFormat.prototype._compileToken = function(ast, lc, data) {
  if (typeof ast == 'string') return JSON.stringify(ast);

  var fn, args;
  switch (ast.type) {
    case 'argument':
      var ret = propname(ast.arg, 'd');
      return this.bidiSupport ? bidiMarkText(ret, lc) : ret;

    case 'select':
      data.keys[data.pf_count] = ast.arg;
      data.offset[data.pf_count] = 0;
      fn = 'select';
      args = [ propname(ast.arg, 'd'), this._compileCases(ast, lc, data) ];
    ...
    case 'selectordinal':
      data.keys[data.pf_count] = ast.arg;
      fn = 'plural';
      args = [ propname(ast.arg, 'd'), 0, funcname(lc), this._compileCases(ast, lc, data), 1 ];
    ...
    case 'plural':
      ...
      fn = 'plural';
      args = [ propname(ast.arg, 'd'), ast.offset || 0, funcname(lc), this._compileCases(ast, lc, data) ];
      this._rt.mf.plural = true;
      this._rt.pf[lc] = true;
      break;
    ...
    case 'function':
      ...
      if (this.intlSupport && !(ast.key in this.fmt) && (ast.key in MessageFormat.formatters)) {
        var tmp = MessageFormat.formatters[ast.key];
      ...
      args = [ propname(ast.arg, 'd'), JSON.stringify(lc) ];
      if (ast.params && ast.params.length) args.push(JSON.stringify(ast.params.length == 1 ? ast.params[0] : ast.params));
      this._rt.fmt[ast.key] = true;
      fn = propname(ast.key, 'fmt');
      break;
  ...
  if (!fn) throw new Error('Parser error for AST ' + JSON.stringify(ast));
  return fn + '(' + args.join(', ') + ')';
};
...
MessageFormat.prototype._compileObj = function(obj, lc, pf) {
  if (typeof obj != 'object') {
    ...
    var data = { keys: {}, offset: {}, pf_count: 0 };
    var r = [];
    for (var i = 0; i < ast.length; ++i ) {
      r.push(this._compileToken(ast[i], lc, data));
    }
