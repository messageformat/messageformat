...
MessageFormat.prototype.compile = function(messages, opt) {
  var r = {},
      compileMsg = function(self, msg, lc) {
        try {
          var ast = MessageFormat._parse(msg);
          return self._precompile(ast, lc);
        } catch (e) {
  ...
  if (typeof messages == 'string') {
    var f = new Function(
        ...
        'return ' + compileMsg(this, messages, this.lc[0]));
  ...
  for (var ns in messages) {
    var lc = opt.locale && opt.locale[ns] || opt.locale || this.lc[0];
    if (typeof messages[ns] == 'string') {
      try { r[ns] = compileMsg(this, messages[ns], lc); }
      catch (e) { e.message = e.message.replace(':', ' with `' + ns + '`:'); throw e; }
    } else {
      r[ns] = {};
      for (var key in messages[ns]) {
        try { r[ns][key] = compileMsg(this, messages[ns][key], lc); }
