 ...
 *
 *  The custom formatting functions are called by `{var, fn[, args]*}` syntax.
 *  For examples, see {@link MessageFormat.formatters}
 *
 * @class
 * @param {string|string[]|Object.<string,function>} [locale="en"] - The locale(s) to use
 ...
 *
 *  Precompiled from `lib/parser.pegjs` by
    ...
    case 'elementFormat':
      ...
      switch (ast.key) {
        ...
        case 'plural':
          ...
          return 'plural(' + args.join(', ') + ')';
        default:
          if (this.withIntlSupport && !(ast.key in this.fmt) && (ast.key in MessageFormat.formatters)) {
...
MessageFormat.prototype.compile = function(messages, locale) {
  ...
  var pf = this.pluralFuncs;
  if (locale) {
    pf = {};
    pf[locale] = this.pluralFuncs[locale] || getPluralFunc(locale);
  } else if (Object.keys(pf).length == 0) {
