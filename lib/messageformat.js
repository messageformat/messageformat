 ...
 *
 *  The optional custom formatting functions are available in MessageFormat
 *  using `{var, fn[, args]*}` syntax. For examples, see {@link MessageFormat.formatters}
 *
 * @class
 * @param {string|string[]|Object.<string,function>} [locale] - The locale(s) to use
 * @param {Object.<string,function>} [formatters] - Optional custom formatting functions
 */
function MessageFormat(locale, formatters) {
  this.pluralFuncs = {};
  if (locale) {
 ...
 *
 *  Precompiled from `lib/parser.pegjs` by {@link http://pegjs.org/ PEG.js}.
 *  Included in MessageFormat object to enable testing.
 *
 * @private
 */
MessageFormat._parse = require('./parser').parse;
 ...
 *
 *  To define your own formatting function, add it to `MessageFormat#fmt` or
 *  include it in the MessageFormat constructor. In JavaScript, each formatting
 *  function is called with three parameters; the variable value `v`, the
 *  current locale `lc`, and (if set) `args` as a single string, or an array of
 *  strings.
 *
 ...
 *
 * @example
 * var fmt = { uppercase: function(v) { return v.toUpperCase(); } };
 * var mf = new MessageFormat('en-GB', fmt);
 *
 * mf.compile('This is {VAR, uppercase}.')({ VAR: 'big' })
 * // 'This is BIG.'
 *
 * mf.fmt.locale = function(v, lc) { return lc; };
 * mf.compile('The current locale is {_, locale}.')({ _: '' })
 * // 'The current locale is en-GB.'
 *
 * mf.fmt.prop = function(v, lc, p) { return v[p] };
...
MessageFormat.runtime = {
  ...
  /** @private */
  toString: function(pluralFuncs, fmt, rt) {
    ...
    var obj = {};
    var pf = Object.keys(rt.pf);
    if (pf.length) obj.pluralFuncs = pf.reduce(function(o, lc) {
      o[lc] = pluralFuncs[lc];
      return o;
    }, {});

    var fmt = Object.keys(rt.fmt);
    if (fmt.length) obj.fmt = fmt.reduce(function(o, key) {
      o[key] = fmt[key];
      return o;
    }, {});

    Object.keys(rt.mf).forEach(function(fn) {
      obj[fn] = MessageFormat.runtime[fn];
    });
 ...
 * @param {string} lc - the current locale code
 * @private
 */
MessageFormat.prototype._precompile = function(ast, lc, data) {
  ...
  switch ( ast.type ) {
    case 'messageFormatPattern':
      for ( i = 0; i < ast.statements.length; ++i ) {
        r.push(this._precompile(ast.statements[i], lc, data));
      }
      tmp = r.join(' + ') || '""';
    ...
    case 'messageFormatElement':
      ...
      if ( ast.output ) {
        ...
        return this.bidiSupport ? bidiMarkText(ret, lc) : ret;
      }
      else {
        data.keys[data.pf_count] = ast.argumentIndex;
        return this._precompile(ast.elementFormat, lc, data);
      }
    ...
    case 'elementFormat':
      ...
      switch (ast.key) {
        case 'select':
          args.push(this._precompile(ast.val, lc, data));
          this._rt.mf.select = true;
          return 'select(' + args.join(', ') + ')';
        case 'selectordinal':
          args = args.concat([ 0, propname(lc, 'pluralFuncs'), this._precompile(ast.val, lc, data), 1 ]);
          this._rt.mf.plural = true;
          this._rt.pf[lc] = true;
          return 'plural(' + args.join(', ') + ')';
        case 'plural':
          data.offset[data.pf_count || 0] = ast.val.offset || 0;
          args = args.concat([ data.offset[data.pf_count] || 0, propname(lc, 'pluralFuncs'), this._precompile(ast.val, lc, data) ]);
          this._rt.mf.plural = true;
          this._rt.pf[lc] = true;
          return 'plural(' + args.join(', ') + ')';
        default:
          if (this.intlSupport && !(ast.key in this.fmt) && (ast.key in MessageFormat.formatters)) {
            tmp = MessageFormat.formatters[ast.key];
            this.fmt[ast.key] = (typeof tmp(this) == 'function') ? tmp(this) : tmp;
          }
          if (!this.fmt[ast.key]) throw new Error('Formatting function ' + JSON.stringify(ast.key) + ' not found!');
          args.push(JSON.stringify(lc));
          if (ast.val && ast.val.length) args.push(JSON.stringify(ast.val.length == 1 ? ast.val[0] : ast.val));
          this._rt.fmt[ast.key] = true;
    ...
    case 'selectFormatPattern':
      ...
      for (i = 0; i < ast.pluralForms.length; ++i) {
        ...
        data_copy.pf_count++;
        r.push(propname(key) + ': function() { return ' + this._precompile(ast.pluralForms[i].val, lc, data_copy) + ';}');
      }
...
/** @private */
MessageFormat.prototype._compileObj = function(obj, lc, pf) {
  if (typeof obj != 'object') {
    var ast = MessageFormat._parse(obj);
    return this._precompile(ast, lc);
  }
  var result = {};
  for (var key in obj) {
    if (pf.hasOwnProperty(key)) lc = key;
    result[key] = this._compileObj(obj[key], lc, pf);
  }
  return result;
}
...
MessageFormat.prototype.compile = function(messages, locale) {
  function _stringify(obj, level) {
    ...
    for (var k in obj) o.push('\n' + indent + '  ' + propname(k) + ': ' + _stringify(obj[k], level + 1));
    return '{' + o.join(',') + '\n' + indent + '}';
  }
  ...
  if (Object.keys(this.pluralFuncs).length == 0) {
    ...
    pf = {};
    pf[locale] = getPluralFunc(locale);
  } else if (locale) {
    pf = {};
  ...
  this._rt = { pf: {}, mf: {}, fmt: {} };
  var obj = this._compileObj(messages, locale, pf);
  ...
  if (typeof messages != 'object') {
    ...
    return fn(rt.number, rt.plural, rt.select, pf, this.fmt);
  }

  var rtStr = MessageFormat.runtime.toString(pf, this.fmt, this._rt) + '\n';
  ...
  result.toString = function(global) {
    switch (global || '') {
      case 'exports':
        var o = [];
        for (var k in obj) o.push(propname(k, 'exports') + ' = ' + _stringify(obj[k]));
        return rtStr + o.join(';\n');
      case 'module.exports':
