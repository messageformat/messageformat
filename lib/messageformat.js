...
/** @private */
MessageFormat.prototype._compileObj = function(obj, lc) {
  ...
  var result = {};
  for (var key in obj) {
    if (this.lc.indexOf(key) !== -1) lc = key;
    result[key] = this._compileObj(obj[key], lc);
  }
  return result;
}
...
MessageFormat.prototype.compile = function(messages, locale) {
  ...
  var lc = locale || this.lc[0];
  var obj = this._compileObj(messages, lc);
  ...
  if (typeof messages != 'object') {
    var fn = new Function(
        ...
        'return ' + obj);
    var rt = this.runtime;
    return fn(rt.number, rt.plural, rt.select, rt.pluralFuncs, rt.fmt);
  }

  var rtStr = this.runtime.toString() + '\n';
