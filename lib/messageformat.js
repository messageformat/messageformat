...
MessageFormat.runtime = {
  ...
  toString: function(pluralFuncs, fmt, rt) {
    ...
    var obj = {};
    Object.keys(rt.pf).forEach(function(lc) { obj[funcname(lc)] = pluralFuncs[lc]; });
    Object.keys(rt.mf).forEach(function(fn) { obj[fn] = MessageFormat.runtime[fn]; });
    var fmt = Object.keys(rt.fmt);
    if (fmt.length) obj.fmt = fmt.reduce(function(o, key) { o[key] = fmt[key]; return o; }, {});
...
/** @private */
MessageFormat.prototype._compileCases = function(token, lc, plural) {
  var needOther = true;
  var r = token.cases.map(function(c) {
    if (c.key === 'other') needOther = false;
    var s = c.tokens.map(function(t) { return this._compileToken(t, lc, plural); }, this);
    return propname(c.key) + ': function(){ return ' + (s.join(' + ') || '""') + ';}';
  }, this);
  if (needOther) throw new Error("No 'other' form found in " + token.type + " " + data.pf_count);
  return '{ ' + r.join(', ') + ' }';
}

/** Recursively map a token to its resulting string
 *
 * @memberof MessageFormat
 * @param {string|Object} token - the token for which the JS code should be generated
 * @param {string} lc - the current locale code
 * @param plural - if set, the nearest surrounding plural or selectordinal token
 * @private
 */
MessageFormat.prototype._compileToken = function(token, lc, plural) {
  if (typeof token == 'string') return JSON.stringify(token);

  var fn, args = [ propname(token.arg, 'd') ];
  switch (token.type) {
    case 'argument':
      return this.bidiSupport ? bidiMarkText(args[0], lc) : args[0];

    case 'select':
      fn = 'select';
      args.push(this._compileCases(token, lc, plural));
      this._rt.mf.select = true;
      break;

    case 'selectordinal':
      fn = 'plural';
      args = [ args[0], 0, funcname(lc), this._compileCases(token, lc, token), 1 ];
    ...
    case 'plural':
      fn = 'plural';
      args = [ args[0], token.offset || 0, funcname(lc), this._compileCases(token, lc, token) ];
      this._rt.mf.plural = true;
      this._rt.pf[lc] = true;
      break;

    case 'function':
      if (this.intlSupport && !(token.key in this.fmt) && (token.key in MessageFormat.formatters)) {
        var tmp = MessageFormat.formatters[token.key];
        this.fmt[token.key] = (typeof tmp(this) == 'function') ? tmp(this) : tmp;
      }
      if (!this.fmt[token.key]) throw new Error('Formatting function ' + JSON.stringify(token.key) + ' not found!');
      ...
      this._rt.fmt[token.key] = true;
      break;

    case 'octothorpe':
      if (!plural) return '"#"';
      fn = 'number';
      args = [ propname(plural.arg, 'd') ];
...
MessageFormat.prototype._compileObj = function(obj, lc, pf) {
  if (typeof obj != 'object') {
    var r = MessageFormat._parse(obj).map(function(token) { return this._compileToken(token, lc); }, this);
...
MessageFormat.prototype.compile = function(messages, locale) {
  ...
  if (typeof messages != 'object') {
    var fn = new Function(
        'number, plural, select, fmt', funcname(locale),
        'return ' + obj);
